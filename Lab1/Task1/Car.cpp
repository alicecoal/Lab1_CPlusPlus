...
int DoorIsOpen = 0, k = 0; //индикатор открытой двери
int light = 0; //индикатор света фар

void __fastcall TForm2::FormPaint(TObject *Sender)//построение рисунка машины
{
...
//---------------------------------------------------------------------------
void __fastcall TForm2::Button2Click(TObject *Sender)  //моргание фар
{
	if (light == 0){
...
{
	if (DoorIsOpen==0){ //если дверь закрыта
		door->erase();
		k=1;
